...
-- questions stay in the existing table; now every member can read them
create policy "members read all questions" on questions
  for select using (has_vault_access(auth.uid()));
...
begin
  ...
  if not exists (select 1 from questions q where q.id = p_question) then return; end if;
