...
/* ---------- the painted world ---------- */
const MAP_IMG = 'map-art/map-final-v6.webp';
...
export const BUILD = 'art-2026-08-23-h';
...
export async function mount(container, api, opts) {
  ...
  const screenSets = SCREENS.map((sc) => {
    ...
    const frames = sc.imgs.map((name, i) => {
      ...
      im.src = 'map-art/screens/' + name + '?v=2';   // the A+ order changed once; the name did not
